...
<% } %>
	...
	loadLicenceRenewalHistoryList = function() {
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("라이센스 갱신 목록 조회", "라이센스 갱신 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
				...
				htmlContents += '<th width="125" id="CREATEDATETIME" class="ui-state-default">갱신일시</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
				...
				if (resultRecordCount > 0) {
					...
					htmlContents += '<tr>';
					htmlContents += '<td colspan="8" align="center"><div style="padding: 10px 0; text-align: center;">등록된 라이센스 갱신 목록이 존재하지 않습니다.</div></td>';
					htmlContents += '</tr>';
				...
				if (objTreeReference.get_selected().attr('node_type') == "company") {
					$('.inner-center .pane-header').text('라이센스 갱신 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 라이센스 갱신 목록');
				}
				g_objLicenceRenewalHistoryList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("라이센스 갱신 목록 조회", "라이센스 갱신 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
...
<div class="inner-center"> 
	<div class="pane-header">전체 라이센스 갱신 목록</div>
