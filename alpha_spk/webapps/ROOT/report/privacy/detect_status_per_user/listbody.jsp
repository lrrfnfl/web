...
<!--
	...
	var g_objDetectStatusPerUserList;
	...
	$(document).ready(function() {
		...
		g_objDetectStatusPerUserList = $('#detectstatusperuser-list');
...
<% } %>
		...
		fillDropdownList(g_objDetectStatusPerUserList.find('#search-condition').find('select[name="searchsearchtype"]'), g_htSearchTypeList, null, "전체");
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				startLoadDetectStatusPerUserList();
			} else if ($(this).attr('id') == 'btnDownload') {
				downloadDetectStatusPerUserList();
			}
		});
...
<% } %>
	...
	startLoadDetectStatusPerUserList = function() {
		...
		setTimeout(function() { loadDetectStatusPerUserList(); }, 50);
	};

	loadDetectStatusPerUserList = function() {

		var objSearchCondition = g_objDetectStatusPerUserList.find('#search-condition');
		var objSearchResult = g_objDetectStatusPerUserList.find('#search-result');
		...
		var postData = getRequestDetectStatusPerUserListParam(
				targetCompanyId,
				arrTargetDeptList,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("사용자별 검출 현황 조회", "사용자별 검출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th id="DETECTFILECOUNT" class="ui-state-default">검출 파일수</th>';
				htmlContents += '<th id="DETECTPATTERNCOUNT" class="ui-state-default">검출 패턴 유형</th>';
				htmlContents += '<th id="DETECTKEYWORDCOUNT" class="ui-state-default">검출 패턴 수</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
...
<% } %>
						...
						htmlContents += '<td style="text-align: right;">' + detectFileCount.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' 파일</td>';
						htmlContents += '<td style="text-align: right;">' + detectPatternCount.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' 패턴</td>';
						htmlContents += '<td style="text-align: right;">' + detectKeywordCount.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' 건</td>';
						htmlContents += '</tr>';
					});
...
<% } %>
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="6" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자별 검출 현황이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="5" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자별 검출 현황이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 사용자별 검출 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 사용자별 검출 현황');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 사용자별 검출 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 사용자별 검출 현황');
				}

				g_objDetectStatusPerUserList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("사용자별 검출 현황 조회", "사용자별 검출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadDetectStatusPerUserList = function() {
		...
		var postData = getRequestCreateDetectStatusPerUserListFileParam(
				targetCompanyId,
				arrTargetDeptList,
				objSearchSearchType.val(),
				objSearchDateFrom.val(),
...
<div class="inner-center"> 
	...
	<div class="ui-layout-content">
		<div id="detectstatusperuser-list" style="display: none;">
