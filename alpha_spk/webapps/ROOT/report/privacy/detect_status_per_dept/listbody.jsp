...
<!--
	...
	var g_objDetectStatusPerDeptList;
	...
	$(document).ready(function() {
		...
		g_objDetectStatusPerDeptList = $('#detectstatusperdept-list');
...
<% } %>
		...
		fillDropdownList(g_objDetectStatusPerDeptList.find('#search-condition').find('select[name="searchsearchtype"]'), g_htSearchTypeList, null, "전체");

<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
		innerDefaultLayout.show("west");
		loadCompanyTreeView();
<% } else { %>
		loadDetectStatusPerDeptList();
<% } %>
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				downloadDetectStatusPerDeptList();
			}
		});
...
<% } %>
	...
	loadDetectStatusPerDeptList = function() {

		var objSearchCondition = g_objDetectStatusPerDeptList.find('#search-condition');
		var objSearchResult = g_objDetectStatusPerDeptList.find('#search-result');
...
<% } %>
		...
		var postData = getRequestDetectStatusPerDeptListParam(
				targetCompanyId,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("부서별 검출 현황 조회", "부서별 검출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th id="USERCOUNT" class="ui-state-default">검출 사용자수</th>';
...
<% } %>
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="6" align="center"><div style="padding: 10px 0; text-align: center;">등록된 부서별 검출 현황이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="5" align="center"><div style="padding: 10px 0; text-align: center;">등록된 부서별 검출 현황이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
				if (objTreeReference.get_selected().attr("node_type") == 'company') {
					$('.inner-center .pane-header').text('사업장 부서별 검출 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 부서별 검출 현황');
				}
<% } else { %>
				$('.inner-center .pane-header').text('부서별 검출 현황');
<% } %>
				g_objDetectStatusPerDeptList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("부서별 검출 현황 조회", "부서별 검출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadDetectStatusPerDeptList = function() {
...
<% } %>
		...
		var postData = getRequestCreateDetectStatusPerDeptListFileParam(
				targetCompanyId,
				objSearchSearchType.val(),
				objSearchDateFrom.val(),
				objSearchDateTo.val(),
...
<div class="inner-center">
	...
	<div class="ui-layout-content">
		<div id="detectstatusperdept-list" style="display: none;">
