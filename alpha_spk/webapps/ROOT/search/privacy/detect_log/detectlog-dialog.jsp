...
<style>
	#tab-detectloginfo > .ui-widget-header { 
		border: none;
...
<!--
	...
	openDetectLogInfoDialog = function(seqNo) {

		var postData = getRequestDetectLogInfoParam(seqNo);
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("검출 정보 조회", "검출 정보 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
				...
				var dialogOptions = {
					...
					"resizeStop" : function(event, ui) {
						...
						if (g_selectedDialogTabIndex == 0) {
							resizeDialogElement($(this), $(this).find('#detectinfo'));
						} else {
				...
				var dialogContents = "";
				dialogContents = '<div title="검출 정보" class="dialog-form">';
				dialogContents += '<div class="dialog-contents">';
				dialogContents += '<div id="tab-detectloginfo" class="styles-tab" style="border: none;">';
				dialogContents += '<ul>';
				dialogContents += '<li><a href="#tab-detectinfo">검출 정보</a></li>';
				dialogContents += '<li><a href="#tab-detectdetailinfo">검출 상세 정보</a></li>';
				dialogContents += '</ul>';
				dialogContents += '<div id="tab-detectinfo" style="padding: 8px 2px 0 2px;">';
				dialogContents += '<div id="detectinfo">';
				...
				dialogContents += '</div>';
				dialogContents += '<div id="row-searchdate" class="field-line">';
				dialogContents += '<div class="field-title">검사 일자</div>';
				dialogContents += '<div class="field-value"><span id="searchdate">' + $(data).find('searchdate').text() + '</span></div>';
				dialogContents += '</div>';
				...
				dialogContents += '</div>';
				dialogContents += '<div id="row-detectstatus" class="field-line">';
				dialogContents += '<div class="field-title">검출 상태</div>';
				dialogContents += '<div class="field-value"><span id="detectstatus">' + g_htDetectStateList.get($(data).find('detectstatus').text()) + '</span></div>';
				dialogContents += '</div>';
				dialogContents += '<div id="row-resultstate" class="field-line">';
				dialogContents += '<div class="field-title">처리 상태</div>';
				dialogContents += '<div class="field-value"><span id="resultstate">' + g_htResultStateList.get($(data).find('result').text()) + '</span></div>';
				dialogContents += '</div>';
				...
				dialogContents += '</div>';
				dialogContents += '</div>';
				dialogContents += '<div id="tab-detectdetailinfo" style="padding: 8px 2px 0 2px;">';
				...
				newDialog.find('.scroll-table').scrolltable({
					stripe: true,
					oddClass: 'odd',
					height: newDialog.find('#tab-detectinfo').height() - 26
				});

				newDialog.find('#tab-detectloginfo').tabs({
					heightStyle: "content",
					active: 0,
					activate: function(event, ui) {
						g_selectedDialogTabIndex = ui.newTab.index();
						if (g_selectedDialogTabIndex == 0) {
							resizeDialogElement(newDialog, newDialog.find('#detectinfo'));
						} else {
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("검출 정보 조회", "검출 정보 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
