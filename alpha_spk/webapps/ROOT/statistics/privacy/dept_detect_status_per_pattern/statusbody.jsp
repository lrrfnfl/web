...
<% } %>
		...
		var postData = getRequestDeptDetectStatusPerPatternParam(
				targetPatternId,
				targetPatternSubId,
				companyId,
				objSearchType.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("패턴별 부서 검출 현황 조회", "패턴별 부서 검출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("패턴별 부서 검출 현황 조회", "패턴별 부서 검출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
...
<% } %>
		...
		var postData = getRequestCreateDeptDetectStatusPerPatternFileParam(
				targetPatternId,
				targetPatternSubId,
				companyId,
				objSearchType.val(),
		...
		var downloadFileName = "패턴별 부서 검출 현황";
		downloadFileName += " (" + objSearchDateFrom.val() + "~" + objSearchDateTo.val() + ")";

		openDownloadDialog("패턴별 부서 검출 현황", downloadFileName, postData);
	};
//-->
</script>
