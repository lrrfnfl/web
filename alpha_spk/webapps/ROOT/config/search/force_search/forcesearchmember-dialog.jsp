...
<!--
	var g_objForceSearchMemberTree;
	var g_objForceSearchMemberDialog;
	var g_bReloadForceSearchMemberTree = false;

	$(document).ready(function() {
		g_objForceSearchMemberDialog = $('#dialog-forcesearchmember');
		g_objForceSearchMemberTree = $('#forcesearchmember-tree');

		g_objForceSearchMemberDialog.dialog({
			autoOpen: false,
			minWidth: 400,
			...
			close: function() {
				...
				g_objForceSearchDialog.find('#targetusercount').text(targetUserCount.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
			},
		...
		g_objForceSearchMemberDialog.find('input[name="includechilddept"]:checkbox').click(function() {
			loadForceSearchMemberList();
		});
	});

	openForceSearchMemberDialog = function() {
		g_objForceSearchMemberDialog.dialog('open');
	};
	...
	loadForceSearchMemberTreeView = function() {

		var xmlTreeData = loadCompanyNode(g_objForceSearchDialog.find('input[name="companyid"]').val());

		g_objForceSearchMemberTree.jstree({
			"xml_data" : {
				"data" : xmlTreeData,
				"ajax": {
					"type": 'POST',
...
<% } %>
	...
	loadForceSearchMemberList = function() {

		if (g_objForceSearchDialog.find('#searchid').text().length <= 0)
			return false;

		var postData = getRequestForceSearchMemberListParam(
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("강제검사 대상 목록 조회", "강제검사 대상 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				} else {
					$(data).find('record').each(function() {
						var targetNodeId = "uid_" + $(this).find('companyid').text() + "_" + $(this).find('userid').text();
						$.jstree._reference(g_objForceSearchMemberTree).check_node('#' + targetNodeId);
					});
				}
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("강제검사 대상 목록 조회", "강제검사 대상 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
...
<div id="dialog-forcesearchmember" title="강제검사 대상 선택" class="dialog-form">
	<div class="dialog-contents">
		<div class="info">
			<ul class="infolist">
				<li>강제검사 대상 사용자를 선택해 주세요.</li>
			</ul>
		</div>
		<div class="category-title">공지 대상 사용자</div>
		<div class="category-contents zero-padding">
			<div id="forcesearchmember-tree" class="treeview-pannel" style="height: 300px;"></div>
		</div>
	</div>
</div>
