...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
						$('.inner-center .pane-header').text('사업장 패턴 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
<% } else { %>
						$('.inner-center .pane-header').text('전체 패턴 검출 현황');
<% } %>
					} else if (data.rslt.obj.attr('node_type') == "dept") {
						$('.inner-center .pane-header').text('부서 패턴 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
					} else if (data.rslt.obj.attr('node_type') == "user") {
						$('.inner-center .pane-header').text('사용자 패턴 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
					} else {
						$('.inner-center .pane-header').text('전체 사업장 패턴 검출 현황');
					}
	...
	loadStatisticsData = function() {
		...
		var postData = getRequestPatternDetectStatusPerOrganizationParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchType.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("조직별 패턴 검출 현황 조회", "조직별 패턴 검출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("조직별 패턴 검출 현황 조회", "조직별 패턴 검출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
	...
	loadChartTable = function(responseData) {
		...
		htmlContents += '<thead>';
		htmlContents += '<tr>';
		htmlContents += '<th class="ui-state-default">검출패턴</th>';
		...
		if (responseData.find('record').length > 0) {
			responseData.find('record').each( function(index) {
				...
				htmlContents += '<td style="text-align: center;">' + patternName + '</td>';
	...
	downloadStatisticsData = function() {
		...
		var postData = getRequestCreatePatternDetectStatusPerOrganizationFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchType.val(),
		...
		var downloadFileName = "패턴 검출 현황";
		downloadFileName += " (" + objSearchDateFrom.val() + "~" + objSearchDateTo.val() + ")";

		openDownloadDialog("패턴 검출 현황", downloadFileName, postData);
	};
//-->
</script>
...
<div class="inner-center">
	<div class="pane-header">전체 사업장 패턴 검출 현황</div>
