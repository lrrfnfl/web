...
<% } %>
			...
			if (!data.rslt.obj.is(g_oldSelectedTreeNode)) {
				if (data.rslt.obj.attr('node_type') == "pattern_category") {
					$('.inner-center .pane-header').text('패턴 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
				} else if (data.rslt.obj.attr('node_type') == "pattern") {
					$('.inner-center .pane-header').text('패턴 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
				} else {
					$('.inner-center .pane-header').text('전체 패턴 검출 현황');
				}
	...
	loadStatisticsData = function() {
		...
		if (objTreeReference.get_selected().attr("node_type") == 'pattern_category') {
			...
			targetPatternId = objTreeReference.get_selected().attr('patternid');
			targetPatternSubId = objTreeReference.get_selected().attr('patternsubid');
		}

		var postData = getRequestCompanyDetectStatusPerPatternParam(
				targetPatternId,
				targetPatternSubId,
				objSearchType.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("패턴별 사업장 검출 현황 조회", "패턴별 사업장 검출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("패턴별 사업장 검출 현황 조회", "패턴별 사업장 검출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
	...
	loadChart = function(responseData) {
		...
		responseData.find('record').each( function(index) {
			if (index < 10) {
				arrlabel.push($(this).find('companyname').text());
	...
	loadChartTable = function(responseData) {
		...
		if (responseData.find('record').length > 0) {
			responseData.find('record').each( function(index) {
				...
				htmlContents += '<td style="text-align: center;">' + companyName + '</td>';
	...
	downloadStatisticsData = function() {
		...
		var postData = getRequestCreateCompanyDetectStatusPerPatternFileParam(
				targetPatternId,
				targetPatternSubId,
				objSearchType.val(),
				objSearchDateFrom.val(),
		...
		var downloadFileName = "패턴별 사업장 검출 현황";
		downloadFileName += " (" + objSearchDateFrom.val() + "~" + objSearchDateTo.val() + ")";

		openDownloadDialog("패턴별 사업장 검출 현황", downloadFileName, postData);
	};
//-->
</script>
...
<div class="inner-center">
	<div class="pane-header">전체 패턴 검출 현황</div>
