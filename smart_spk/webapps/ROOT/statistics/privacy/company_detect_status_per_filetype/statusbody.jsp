...
<% } %>
	...
	loadStatisticsData = function() {
		...
		var postData = getRequestCompanyDetectStatusPerFileTypeParam(
				targetFileType,
				objSearchType.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("피일 유형별 사업장 검출 현황 조회", "피일 유형별 사업장 검출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("피일 유형별 사업장 검출 현황 조회", "피일 유형별 사업장 검출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
	...
	downloadStatisticsData = function() {
		...
		var postData = getRequestCreateCompanyDetectStatusPerFileTypeFileParam(
				targetFileType,
				objSearchType.val(),
				objSearchDateFrom.val(),
				objSearchDateTo.val());

		var downloadFileName = "파일 유형별 사업장 검출 현황";
		downloadFileName += " (" + objSearchDateFrom.val() + "~" + objSearchDateTo.val() + ")";

		openDownloadDialog("파일 유형별 사업장 검출 현황", downloadFileName, postData);
	};
//-->
</script>
