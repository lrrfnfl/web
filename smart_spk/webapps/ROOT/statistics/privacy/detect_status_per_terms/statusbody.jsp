...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
						$('.inner-center .pane-header').text('사업장 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
<% } else { %>
						$('.inner-center .pane-header').text('전체 검출 현황');
<% } %>
					} else if (data.rslt.obj.attr('node_type') == "dept") {
						$('.inner-center .pane-header').text('부서 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
					} else if (data.rslt.obj.attr('node_type') == "user") {
						$('.inner-center .pane-header').text('사용자 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
					} else {
						$('.inner-center .pane-header').text('전체 사업장 검출 현황');
					}
	...
	loadStatisticsData = function() {
		...
		var postData = getRequestDetectStatusPerTermsParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				termType,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("기간별 검출 현황 조회", "기간별 검출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("기간별 검출 현황 조회", "기간별 검출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
...
// 				],
	...
	loadColumnChartTable = function(responseData) {
		...
		htmlContents += '<tr>';
		if (g_selectedTabIndex == TAB_DAILY)
			htmlContents += '<th width="70" class="ui-state-default">검출일자</th>';
		else if (g_selectedTabIndex == TAB_MONTHLY)
			htmlContents += '<th width="70" class="ui-state-default">검출년월</th>';
...
//  				],
	...
	loadDoughnutChartTable = function(responseData) {
		...
		if (g_selectedTabIndex == TAB_DAILY)
			$('#daily-pie-chart-table').html(htmlContents);
		else if (g_selectedTabIndex == TAB_MONTHLY)
			$('#monthly-pie-chart-table').html(htmlContents);
	};
	...
	downloadStatisticsData = function() {
		...
		var postData = getRequestCreateDetectStatusPerTermsFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				termType,
...
<div id="tab-main" class="inner-center styles-tab">
	<div class="pane-header">전체 사업장 검출 현황</div>
	<ul>
		<li><a href="#tab-daily">일별 검출 현황</a></li>
		<li><a href="#tab-monthly">월별 검출 현황</a></li>
	</ul>
	<div class="ui-layout-content">
		<div id="tab-daily" class="zero-padding">
			...
			<div class="sub-category-contents" style="margin-top: 15px;">
				...
				<div style="margin-left: 51%;">
					<div id="daily-pie-chart-table"></div>
				</div>
				<div class="clear"></div>
			</div>
		</div>
		<div id="tab-monthly" class="zero-padding">
			...
			<div class="sub-category-contents" style="margin-top: 15px;">
				...
				<div style="margin-left: 51%;">
					<div id="monthly-pie-chart-table"></div>
				</div>
				<div class="clear"></div>
			</div>
		</div>
