...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
						$('.inner-center .pane-header').text('사업장 랜섬웨어 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
<% } else { %>
						$('.inner-center .pane-header').text('전체 랜섬웨어 검출 현황');
<% } %>
					} else if (data.rslt.obj.attr('node_type') == "dept") {
						$('.inner-center .pane-header').text('부서 랜섬웨어 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
					} else if (data.rslt.obj.attr('node_type') == "user") {
						$('.inner-center .pane-header').text('사용자 랜섬웨어 검출 현황 - [' + data.rslt.obj.children('a').text().trim() + ']');
					} else {
						$('.inner-center .pane-header').text('전체 사업장 랜섬웨어 검출 현황');
					}
	...
	loadStatisticsData = function() {
		...
		var postData = getRequestRansomwareDetectStatusPerTermsParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				termType,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("기간별 랜섬웨어 검출 현황 조회", "기간별 랜섬웨어 검출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("기간별 랜섬웨어 검출 현황 조회", "기간별 랜섬웨어 검출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
	...
	loadChart = function(responseData) {
		...
		responseData.find('record').each( function(index) {
			...
			arrData.push($(this).find('detectcount').text());
		});
...
// 				],
	...
	loadChartTable = function(responseData) {
		...
		htmlContents += '<tr>';
		if (g_selectedTabIndex == TAB_DAILY)
			htmlContents += '<th class="ui-state-default">검출일자</th>';
		else if (g_selectedTabIndex == TAB_MONTHLY)
			htmlContents += '<th class="ui-state-default">검출년월</th>';
		htmlContents += '<th class="ui-state-default">검출 건수</th>';
		htmlContents += '</tr>';
		htmlContents += '</thead>';
		htmlContents += '<tbody>';
		...
		responseData.find('record').each( function(index) {
			...
			var detectCount = parseInt($(this).find('detectcount').text());

			htmlContents += '<tr class="' + lineStyle + '">';
			htmlContents += '<td style="text-align: center;">' + date + '</td>';
			htmlContents += '<td style="text-align: right;">' + detectCount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + '</td>';
			htmlContents += '</tr>';
		});
	...
	downloadStatisticsData = function() {
		...
		var postData = getRequestCreateRansomwareDetectStatusPerTermsFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				termType,
...
<div id="tab-main" class="inner-center styles-tab">
	<div class="pane-header">전체 사업장 랜섬웨어 검출 현황</div>
	<ul>
		<li><a href="#tab-daily">일별 랜섬웨어 검출 현황</a></li>
		<li><a href="#tab-monthly">월별 랜섬웨어 검출 현황</a></li>
	</ul>
