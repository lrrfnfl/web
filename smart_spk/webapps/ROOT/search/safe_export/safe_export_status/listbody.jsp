...
<% } %>
		...
		fillDropdownList(g_objSafeExportList.find('#search-condition').find('select[name="searchdecodestatus"]'), g_htOptionTypeList, null, "전체");
...
<% } %>
	...
	loadSafeExportList = function() {
		...
		if (objSearchReveiver.val().length > 0) {
			if (!isValidParam(objSearchReveiver, PARAM_TYPE_SEARCH_KEYWORD, "수취인", PARAM_SEARCH_KEYWORD_MIN_LEN, PARAM_SEARCH_KEYWORD_MAX_LEN, null)) {
				return false;
			}
		} else {
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("안전반출 목록 조회", "안전반출 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th id="RECEIVER" class="ui-state-default">수취인</th>';
				htmlContents += '<th id="RECEIVEREMAIL" class="ui-state-default">수취인 Email</th>';
				htmlContents += '<th id="EXPORTFILESCOUNT" class="ui-state-default">반출 파일 수</th>';
				htmlContents += '<th id="DECODESTATUS" class="ui-state-default">복호화 상태</th>';
				htmlContents += '<th id="CREATEDATETIME" class="ui-state-default">반출 요청일시</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
...
<% } %>
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="8" align="center"><div style="padding: 10px 0; text-align: center;">등록된 안전반출 목록이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="7" align="center"><div style="padding: 10px 0; text-align: center;">등록된 안전반출 목록이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 안전반출 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 안전반출 목록');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 안전반출 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 안전반출 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 안전반출 목록');
				}
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("안전반출 목록 조회", "안전반출 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
	...
	downloadSafeExportList = function() {
		...
		openDownloadDialog("안전반출 목록", postData);
	};
//-->
</script>
...
<div class="inner-center">
	<div class="pane-header ui-widget-header ui-corner-top">전체 사업장 안전반출 목록</div>
	<div class="ui-layout-content ui-widget-content ui-corner-bottom">
		<div id="safeexport-list" style="display: none;">
			<div id="search-condition" class="inline-search-condition">
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>수취인
				<input type="text" id="searchreceiver" name="searchreceiver" class="text ui-widget-content ui-corner-all" />
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>복호화 상태
