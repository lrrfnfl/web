...
<!--
	...
	$(document).ready(function() {
		...
		g_objMediaControlLogList = $('#mediacontrollog-list');
...
<% } %>
		...
		g_htMediaTypeList = loadTypeList("MEDIA_TYPE");
		if (g_htMediaTypeList.isEmpty()) {
			displayAlertDialog("매체 유형 조회", "매체 유형 조회 중 오류가 발생하였습니다.", "자세한 사항은 시스템 관리자에게 문의해 주십시오.");
		}
		...
		fillDropdownList(g_objMediaControlLogList.find('#search-condition').find('select[name="searchusertype"]'), g_htUserTypeList, null, "전체");
		fillDropdownList(g_objMediaControlLogList.find('#search-condition').find('select[name="searchmediatype"]'), g_htMediaTypeList, null, "전체");
		fillDropdownList(g_objMediaControlLogList.find('#search-condition').find('select[name="searchcontroltype"]'), g_htControlTypeList, null, "전체");
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				loadMediaControlLogList();
			} else if ($(this).attr('id') == 'btnDownload') {
				downloadMediaControlLogList();
			}
		});
		...
		$(document).on("click", ".list-table tbody tr", function() { if (typeof $(this).attr('seqno') != typeof undefined) { openMediaControlLogInfoDialog($(this).attr('seqno')); } });
	});
...
<% } %>
	...
	loadMediaControlLogList = function() {

		var objSearchCondition = g_objMediaControlLogList.find('#search-condition');
		var objSearchResult = g_objMediaControlLogList.find('#search-result');

		var objSearchUserType = objSearchCondition.find('select[name="searchusertype"]');
		var objSearchMediaType = objSearchCondition.find('select[name="searchmediatype"]');
		var objSearchControlType = objSearchCondition.find('select[name="searchcontroltype"]');
		...
		var postData = getRequestMediaControlLogListParam(targetCompanyId,
			arrTargetDeptList,
			targetUserId,
			objSearchUserType.val(),
			objSearchMediaType.val(),
			objSearchControlType.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("매체 제어 목록 조회", "매체 제어 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th width="65" class="ui-state-default">매체 유형</th>';
				htmlContents += '<th width="80" class="ui-state-default">제어 유형</th>';
				htmlContents += '<th width="125" class="ui-state-default">등록 일시</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
...
<% } %>
						...
						htmlContents += '<td>' + g_htMediaTypeList.get(mediaType) + '</td>';
						htmlContents += '<td>' + g_htControlTypeList.get(controlType) + '</td>';
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="8" align="center"><div style="padding: 10px 0; text-align: center;">등록된 매체 제어 목록이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="7" align="center"><div style="padding: 10px 0; text-align: center;">등록된 매체 제어 목록이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 매체 제어 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 매체 제어 목록');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 매체 제어 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 매체 제어 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 매체 제어 목록');
				}

				g_objMediaControlLogList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("매체 제어 목록 조회", "매체 제어 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadMediaControlLogList = function() {
		...
		var postData = getRequestCreateMediaControlLogListFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchUserType.val(),
...
<div class="inner-center">
	<div class="pane-header">전체 사업장 매체 제어 목록</div>
	<div class="ui-layout-content">
		<div id="mediacontrollog-list" style="display: none;">
			<div id="search-condition" class="inline-search-condition">
				...
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>매체 유형
				<select id="searchmediatype" name="searchmediatype" class="ui-widget-content"></select>
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>제어 유형
				<select id="searchcontroltype" name="searchcontroltype" class="ui-widget-content"></select>
