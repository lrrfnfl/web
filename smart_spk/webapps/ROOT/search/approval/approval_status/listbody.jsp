...
<%@ include file ="decodingapproval-dialog.jsp"%>
<script type="text/javascript">
<!--
	var g_objOrganizationTree;
	var g_objDecodingApprovalList;

	var g_htDecodingApprobatorTypeList = new Hashtable();
	var g_htDecodingApprovalKindList = new Hashtable();
	var g_htDecodingApprovalTypeList = new Hashtable();
	var g_htDecodingApprovalStateList = new Hashtable();
	...
	$(document).ready(function() {
		...
		g_objDecodingApprovalList = $('#decodingapproval-list');
...
<% } %>
		...
		g_htDecodingApprobatorTypeList = loadTypeList("DECODING_APPROBATOR_TYPE");
		if (g_htDecodingApprobatorTypeList.isEmpty()) {
			displayAlertDialog("결재자 유형 조회", "결재자 유형 조회 중 오류가 발생하였습니다.", "자세한 사항은 시스템 관리자에게 문의해 주십시오.");
		}

		g_htDecodingApprovalKindList = loadTypeList("DECODING_APPROVAL_KIND");
		if (g_htDecodingApprovalKindList.isEmpty()) {
			displayAlertDialog("결재 종류 조회", "결재 종류 조회 중 오류가 발생하였습니다.", "자세한 사항은 시스템 관리자에게 문의해 주십시오.");
		}

		g_htDecodingApprovalTypeList = loadTypeList("DECODING_APPROVAL_TYPE");
		if (g_htDecodingApprovalTypeList.isEmpty()) {
			displayAlertDialog("결재 유형 조회", "결재 유형 조회 중 오류가 발생하였습니다.", "자세한 사항은 시스템 관리자에게 문의해 주십시오.");
		}

		g_htDecodingApprovalStateList = loadTypeList("DECODING_APPROVAL_STATE");
		if (g_htDecodingApprovalStateList.isEmpty()) {
			displayAlertDialog("결재 상태 유형 조회", "결재 상태 유형 조회 중 오류가 발생하였습니다.", "자세한 사항은 시스템 관리자에게 문의해 주십시오.");
		}

		fillDropdownList(g_objDecodingApprovalList.find('#search-condition').find('select[name="searchdecodingapprovalkind"]'), g_htDecodingApprovalKindList, null, "전체");
		fillDropdownList(g_objDecodingApprovalList.find('#search-condition').find('select[name="searchdecodingapprovaltype"]'), g_htDecodingApprovalTypeList, null, "전체");
		fillDropdownList(g_objDecodingApprovalList.find('#search-condition').find('select[name="searchdecodingapprovalstate"]'), g_htDecodingApprovalStateList, null, "전체");
		g_objDecodingApprovalList.find('#search-condition').find('select[name="searchdecodingapprovalstate"]').find('option[value="' + '<%=DecodingApprovalState.DECODING_APPROVAL_STATE_READY%>' + '"]').remove();
		g_objDecodingApprovalList.find('#search-condition').find('select[name="searchdecodingapprovalstate"]').find('option[value="' + '<%=DecodingApprovalState.DECODING_APPROVAL_STATE_SUBSTITUTE%>' + '"]').remove();
		g_objDecodingApprovalList.find('#search-condition').find('select[name="searchdecodingapprovalstate"]').find('option[value="' + '<%=DecodingApprovalState.DECODING_APPROVAL_STATE_HIGH_RANK%>' + '"]').remove();
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				loadDecodingApprovalList();
			} else if ($(this).attr('id') == 'btnDownload') {
				downloadDecodingApprovalList();
			}
		});
		...
		$(document).on("click", ".list-table tbody tr", function() { if (typeof $(this).attr('seqno') != typeof undefined) { openDecodingApprovalInfoDialog($(this).attr('seqno')); } });
	});
...
<% } %>
	...
	loadDecodingApprovalList = function() {

		var objSearchCondition = g_objDecodingApprovalList.find('#search-condition');
		var objSearchResult = g_objDecodingApprovalList.find('#search-result');

		var objSearchDecodingApprovalKind = objSearchCondition.find('select[name="searchdecodingapprovalkind"]');
		var objSearchDecodingApprovalType = objSearchCondition.find('select[name="searchdecodingapprovaltype"]');
		var objSearchDecodingApprovalState = objSearchCondition.find('select[name="searchdecodingapprovalstate"]');
		...
		var postData = getRequestDecodingApprovalListParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchDecodingApprovalKind.val(),
				objSearchDecodingApprovalType.val(),
				objSearchDecodingApprovalState.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("결재 목록 조회", "결재 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th width="70" class="ui-state-default">결재 종류</th>';
				htmlContents += '<th width="70" class="ui-state-default">결재 유형</th>';
				htmlContents += '<th class="ui-state-default">요청 사유</th>';
				htmlContents += '<th width="125" class="ui-state-default">요청 일시</th>';
				htmlContents += '<th width="70" class="ui-state-default">진행 순위</th>';
				htmlContents += '<th width="70" class="ui-state-default">결재 상태</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
				...
				if (resultRecordCount > 0) {
					$(data).find('record').each( function(index) {
						...
						var approvalKind = $(this).find('approvalkind').text();
						var approvalType = $(this).find('approvaltype').text();
						var reason = $(this).find('reason').text();
						var requestDatetime = $(this).find('requestdatetime').text();
						var priorityInProgress = $(this).find('priorityinprogress').text();
...
<% } %>
						...
						htmlContents += '<td>' + g_htDecodingApprovalKindList.get(approvalKind) + '</td>';
						htmlContents += '<td>' + g_htDecodingApprovalTypeList.get(approvalType) + '</td>';
						htmlContents += '<td style="text-align: left;" title="' + reason + '">' + reason + '</td>';
						htmlContents += '<td>' + requestDatetime + '</td>';
						htmlContents += '<td>' + priorityInProgress + '</td>';
						htmlContents += '<td>' + g_htDecodingApprovalStateList.get(approvalState) + '</td>';
						htmlContents += '</tr>';
					});
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="9" align="center"><div style="padding: 10px 0; text-align: center;">등록된 결재가 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="8" align="center"><div style="padding: 10px 0; text-align: center;">등록된 결재가 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 결재 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 결재 목록');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 결재 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 결재 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 결재 목록');
				}

				g_objDecodingApprovalList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("결재 목록 조회", "결재 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadDecodingApprovalList = function() {
		...
		var postData = getRequestCreateDecodingApprovalListFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchDecodingApprovalKind.val(),
...
<div class="inner-center">
	<div class="pane-header">전체 결재 목록</div>
	<div class="ui-layout-content">
		<div id="decodingapproval-list" style="display: none;">
			<div id="search-condition" class="inline-search-condition">
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>결재 종류
				<select id="searchdecodingapprovalkind" name="searchdecodingapprovalkind" class="ui-widget-content"></select>
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>결재 유형
				<select id="searchdecodingapprovaltype" name="searchdecodingapprovaltype" class="ui-widget-content"></select>
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>결재 상태
				<select id="searchdecodingapprovalstate" name="searchdecodingapprovalstate" class="ui-widget-content"></select>
