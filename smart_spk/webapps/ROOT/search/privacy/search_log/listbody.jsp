...
<!--
	...
	$(document).ready(function() {
		...
		g_objSearchLogList = $('#searchlog-list');
...
<% } %>
		...
		fillDropdownList(g_objSearchLogList.find('#search-condition').find('select[name="searchusertype"]'), g_htUserTypeList, null, "전체");
		fillDropdownList(g_objSearchLogList.find('#search-condition').find('select[name="searchsearchtype"]'), g_htSearchTypeList, null, "전체");
		fillDropdownList(g_objSearchLogList.find('#search-condition').find('select[name="searchdetectestatus"]'), g_htOptionTypeList, null, "전체");
		...
		$(document).on("click", ".list-table tbody tr", function() { if (typeof $(this).attr('seqno') != typeof undefined) { openSearchLogInfoDialog($(this).attr('seqno')); } });
	});
...
<% } %>
	...
	loadSearchLogList = function() {

		var objSearchCondition = g_objSearchLogList.find('#search-condition');
		var objSearchResult = g_objSearchLogList.find('#search-result');
		...
		var objSearchDetecteStatus = objSearchCondition.find('select[name="searchdetectestatus"]');
		...
		var postData = getRequestSearchLogListParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("검사 목록 조회", "검사 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
				...
				htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
				htmlContents += '<th class="ui-state-default">사업장</th>';
<% } %>
				htmlContents += '<th class="ui-state-default">부서</th>';
				htmlContents += '<th class="ui-state-default">사용자</th>';
				htmlContents += '<th class="ui-state-default">사용자 유형</th>';
				htmlContents += '<th class="ui-state-default">검사 유형</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
...
<% } %>
						...
						htmlContents += '<td>' + g_htSearchTypeList.get(searchType) + '</td>';
						htmlContents += '</tr>';
					});
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="5" align="center"><div style="padding: 10px 0; text-align: center;">등록된 검사 목록이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="4" align="center"><div style="padding: 10px 0; text-align: center;">등록된 검사 목록이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 검사 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 검사 목록');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 검사 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 검사 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 검사 목록');
				}

				g_objSearchLogList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("검사 목록 조회", "검사 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadSearchLogList = function() {
		...
		var postData = getRequestCreateSearchLogListFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchSearchId.val(),
...
<div class="inner-center">
	<div class="pane-header">전체 사업장 검사 목록</div>
	<div class="ui-layout-content">
		<div id="searchlog-list" style="display: none;">
			<div id="search-condition" class="inline-search-condition">
				...
				<select id="searchdetectestatus" name="searchdetectestatus" class="ui-widget-content" style="display: none;"></select>
