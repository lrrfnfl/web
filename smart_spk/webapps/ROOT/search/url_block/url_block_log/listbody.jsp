...
<!--
	...
	$(document).ready(function() {
		...
		g_objUrlBlockLogList = $('#urlblocklog-list');
...
<% } %>
		...
		$(document).on("click", ".list-table tbody tr", function() { if (typeof $(this).attr('seqno') != typeof undefined) { openUrlBlockLogInfoDialog($(this).attr('seqno')); } });
	});
...
<% } %>
	...
	loadUrlBlockLogList = function() {

		var objSearchCondition = g_objUrlBlockLogList.find('#search-condition');
		var objSearchResult = g_objUrlBlockLogList.find('#search-result');

		var objSearchBlockUrl = objSearchCondition.find('input[name="searchblockurl"]');
		...
		var postData = getRequestUrlBlockLogListParam(targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchBlockUrl.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("URL 차단 목록 조회", "URL 차단 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th class="ui-state-default">차단 URL</th>';
				htmlContents += '<th class="ui-state-default">차단 내용</th>';
				...
				if (resultRecordCount > 0) {
					$(data).find('record').each( function(index) {
						...
						var blockUrl = $(this).find('blockurl').text();
						var logContents = $(this).find('logcontents').text();
...
<% } %>
						...
						htmlContents += '<td>' + blockUrl + '</td>';
						htmlContents += '<td>' + logContents + '</td>';
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="6 align="center"><div style="padding: 10px 0; text-align: center;">등록된 URL 차단 목록이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="5" align="center"><div style="padding: 10px 0; text-align: center;">등록된 URL 차단 목록이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 URL 차단 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 URL 차단 목록');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 URL 차단 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 URL 차단 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 URL 차단 목록');
				}

				g_objUrlBlockLogList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("URL 차단 목록 조회", "URL 차단 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadUrlBlockLogList = function() {
		...
		var postData = getRequestCreateUrlBlockLogListFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchBlockUrl.val(),
...
<div class="inner-west">
	...
	<div class="ui-layout-content ui-corner-bottom" style="padding: 0;">
		<div id="organization-tree" class="treeview-pannel"></div>
	</div>
</div>
<div class="inner-center">
	<div class="pane-header ui-widget-header ui-corner-top">전체 사업장 URL 차단 목록</div>
	<div class="ui-layout-content ui-widget-content ui-corner-bottom">
		<div id="urlblocklog-list" style="display: none;">
			<div id="search-condition" class="inline-search-condition">
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>차단 URL
				<input type="text" id="searchblockurl" name="searchblockurl" class="text ui-widget-content ui-corner-all" />
