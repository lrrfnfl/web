...
<%@ include file ="userlog-dialog.jsp"%>
<%@ include file ="/include/dialog/download-dialog.jsp"%>
<script type="text/javascript">
<!--
	var g_objOrganizationTree;
	var g_objUserLogList;

	var g_htLogTypeList = new Hashtable();
...
<% } %>
		...
		g_htLogTypeList = loadTypeList("USER_LOG_TYPE");
		if (g_htLogTypeList.isEmpty()) {
			displayAlertDialog("사용자 로그 유형 조회", "사용자 로그 유형 조회 중 오류가 발생하였습니다.", "자세한 사항은 시스템 관리자에게 문의해 주십시오.");
		}

		fillDropdownList(g_objUserLogList.find('#search-condition').find('select[name="searchlogtype"]'), g_htLogTypeList, null, "전체");
		...
		$(document).on("click", ".list-table tbody tr", function() { if (typeof $(this).attr('seqno') != typeof undefined) { openUserLogInfoDialog($(this).attr('seqno')); } });
	});
...
<% } %>
	...
	loadUserLogList = function() {

		var objSearchCondition = g_objUserLogList.find('#search-condition');
		var objSearchResult = g_objUserLogList.find('#search-result');

		var objSearchLogType = objSearchCondition.find('select[name="searchlogtype"]');
		...
		var postData = getRequestUserLogListParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchLogType.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("사용자 로그 목록 조회", "사용자 로그 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th class="ui-state-default">로그 유형</th>';
				htmlContents += '<th class="ui-state-default">로그 일시</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
				...
				if (resultRecordCount > 0) {
					$(data).find('record').each( function(index) {
						...
						var logType = $(this).find('logtype').text();
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
						...
						htmlContents += '<td>' + g_htLogTypeList.get(logType) + '</td>';
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="5" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자 로그가 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="4" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자 로그가 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 사용자 로그 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 사용자 로그 목록');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 사용자 로그 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 로그 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사용자 로그 목록');
				}

				g_objUserLogList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("사용자 로그 목록 조회", "사용자 로그 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadUserLogList = function() {
		...
		var postData = getRequestCreateUserLogListFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchLogType.val(),
...
<div class="inner-center">
	<div class="pane-header">사용자 로그 목록</div>
	<div class="ui-layout-content">
		<div id="userlog-list" style="display: none;">
			<div id="search-condition" class="inline-search-condition">
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>로그 유형
				<select id="searchlogtype" name="searchlogtype" class="ui-widget-content"></select>
