...
<!--
	...
	openPrintLogInfoDialog = function(seqNo) {

		var postData = getRequestPrintLogInfoParam(seqNo);
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("출력 정보 조회", "출력 정보 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
				...
				var dialogOptions = {
					...
					"resizeStop" : function() {
						...
						//resizeDialogElement($(this), $(this).find('#printername'));
					},
				...
				var dialogContents = "";
				dialogContents = '<div title="출력 정보" class="dialog-form">';
				...
				dialogContents += '</div>';
				dialogContents += '<div id="row-printpagecount" class="field-line">';
				dialogContents += '<div class="field-title">출력 페이지 수</div>';
				dialogContents += '<div class="field-value"><span id="printpagecount">' + $(data).find('printpagecount').text().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + '</span></div>';
				dialogContents += '</div>';
				dialogContents += '<div id="row-printername" class="field-line">';
				dialogContents += '<div class="field-title">프린터 명</div>';
				dialogContents += '<div class="field-value"><span id="printername">' + $(data).find('printername').text() + '</span></div>';
				dialogContents += '</div>';
				dialogContents += '<div id="row-printdatetime" class="field-line">';
				dialogContents += '<div class="field-title">출력 일시</div>';
				dialogContents += '<div class="field-contents"><span id="printdatetime">' + $(data).find('printdatetime').text() + '</span></div>';
				dialogContents += '</div>';
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("출력 정보 조회", "출력 정보 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
