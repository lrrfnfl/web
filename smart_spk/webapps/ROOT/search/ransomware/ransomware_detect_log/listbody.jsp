...
<!--
	...
	$(document).ready(function() {
		...
		g_objRansomwareDetectLogList = $('#ransomwaredetectlog-list');
...
<% } %>
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				loadRansomwareDetectLogList();
			} else if ($(this).attr('id') == 'btnDownload') {
				downloadRansomwareDetectLogList();
			}
		});
		...
		$(document).on("click", ".list-table tbody tr", function() { if (typeof $(this).attr('seqno') != typeof undefined) { openRansomwareDetectLogInfoDialog($(this).attr('seqno')); } });
	});
...
<% } %>
	...
	loadRansomwareDetectLogList = function() {

		var objSearchCondition = g_objRansomwareDetectLogList.find('#search-condition');
		var objSearchResult = g_objRansomwareDetectLogList.find('#search-result');
		...
		if (objSearchFileName.val().length > 0) {
			if (!isValidParam(objSearchFileName, PARAM_TYPE_SEARCH_KEYWORD, "파일 명", PARAM_SEARCH_KEYWORD_MIN_LEN, PARAM_SEARCH_KEYWORD_MAX_LEN, null)) {
				return false;
			}
		} else {
		...
		var postData = getRequestRansomwareDetectLogListParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("랜섬웨어 검출 목록 조회", "랜섬웨어 검출 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th class="ui-state-default">검출 코멘트</th>';
				htmlContents += '<th width="125" class="ui-state-default">검출 일시</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
				...
				if (resultRecordCount > 0) {
					$(data).find('record').each( function(index) {
						...
						var detectComments = $(this).find('detectcomments').text();
						var detectDatetime = $(this).find('detectdatetime').text();
...
<% } %>
						...
						htmlContents += '<td style="text-align: left;">' + detectComments + '</td>';
						htmlContents += '<td>' + detectDatetime + '</td>';
						htmlContents += '</tr>';
					});
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="6" align="center"><div style="padding: 10px 0; text-align: center;">등록된 랜섬웨어 검출 목록이 없습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="5" align="center"><div style="padding: 10px 0; text-align: center;">등록된 랜섬웨어 검출 목록이 없습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 랜섬웨어 검출 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 랜섬웨어 검출 목록');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 랜섬웨어 검출 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 랜섬웨어 검출 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 랜섬웨어 검출 목록');
				}

				g_objRansomwareDetectLogList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("랜섬웨어 검출 목록 조회", "랜섬웨어 검출 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadRansomwareDetectLogList = function() {
		...
		var postData = getRequestCreateRansomwareDetectLogListFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchFileName.val(),
...
<div class="inner-center">
	<div class="pane-header">전체 사업장 랜섬웨어 검출 목록</div>
	<div class="ui-layout-content">
		<div id="ransomwaredetectlog-list" style="display: none;">
			<div id="search-condition" class="inline-search-condition">
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>파일 명
