...
<%@ include file ="drmpermissionsettingslog-dialog.jsp"%>
<%@ include file ="/include/dialog/download-dialog.jsp"%>
<script type="text/javascript">
<!--
	var g_objOrganizationTree;
	var g_objDrmPermissionSettingsLogList;
	...
	$(document).ready(function() {
		...
		g_objDrmPermissionSettingsLogList = $('#mediacontrollog-list');
...
<% } %>
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				loadDrmPermissionSettingsLogList();
			}
		});
		...
		$(document).on("click", ".list-table tbody tr", function() { if (typeof $(this).attr('seqno') != typeof undefined) { openDrmPermissionSettingsLogInfoDialog($(this).attr('seqno')); } });
	});
...
<% } %>
	...
	loadDrmPermissionSettingsLogList = function() {

		var objSearchCondition = g_objDrmPermissionSettingsLogList.find('#search-condition');
		var objSearchResult = g_objDrmPermissionSettingsLogList.find('#search-result');
		...
		var postData = getRequestDrmPermissionSettingsLogListParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("문서보안 권한 설정 내역 조회", "문서보안 권한 설정 내역 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th class="ui-state-default">정책명</th>';
				htmlContents += '<th width="85" class="ui-state-default">만기일자</th>';
				htmlContents += '<th width="85" class="ui-state-default">읽기 제한 횟수</th>';
				...
				if (resultRecordCount > 0) {
					$(data).find('record').each( function(index) {
						...
						var readLimitCount = $(this).find('readlimitcount').text();
...
<% } %>
						...
						htmlContents += '<td title="' + fileName + '">' + fileName + '</td>';
						htmlContents += '<td title="' + policyName + '">' + policyName + '</td>';
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="8" align="center"><div style="padding: 10px 0; text-align: center;">등록된 문서보안 권한 설정 내역이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="7" align="center"><div style="padding: 10px 0; text-align: center;">등록된 문서보안 권한 설정 내역이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 문서보안 권한 설정 내역 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 문서보안 권한 설정 내역');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 문서보안 권한 설정 내역 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 문서보안 권한 설정 내역 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 문서보안 권한 설정 내역');
				}

				g_objDrmPermissionSettingsLogList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("문서보안 권한 설정 내역 조회", "문서보안 권한 설정 내역 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
...
<div class="inner-center">
	<div class="pane-header">전체 사업장 문서보안 권한 설정 내역</div>
