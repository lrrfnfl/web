...
<%@ include file ="credentialexception-dialog.jsp"%>
...
<!--
	...
	$(document).ready(function() {
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				loadCredentialExceptionFileList();
			} else if ($(this).attr('id') == 'btnNew') {
				openNewCredentialExceptionDialog();
			} else if ($(this).attr('id') == 'btnDelete') {
				displayConfirmDialog("랜섬웨어 신뢰정보 예외 파일 삭제", "랜섬웨어 신뢰정보 예외 파일을 삭제하시겠습니까?", "", function() { deleteRansomwareCredentialExceptionFile(); } );
			}
		});
	...
	loadFileTreeView = function() {
		...
		xmlTreeData += '<content><name>전체 랜섬웨어 신뢰정보 예외</name></content>';
		xmlTreeData += '</item>';
	...
	loadCredentialExceptionFileList = function() {
		...
		var postData = getRequestRansomwareCredentialExceptionFileListParam(
				categoryCode,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("랜섬웨어 신뢰정보 예외 목록 조회", "랜섬웨어 신뢰정보 예외 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
				...
				if (resultRecordCount >0) {
					...
					htmlContents += '<tr>';
					htmlContents += '<td colspan="2" align="center"><div style="padding: 10px 0; text-align: center;">등록된 랜섬웨어 신뢰정보 예외가 존재하지 않습니다.</div></td>';
					htmlContents += '</tr>';
				...
				$('.inner-center .pane-header').text('전체 랜섬웨어 신뢰정보 예외 목록');
				g_objFileList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("랜섬웨어 신뢰정보 예외 목록 조회", "랜섬웨어 신뢰정보 예외 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	deleteRansomwareCredentialExceptionFile = function() {
		...
		var postData = getRequestDeleteRansomwareCredentialExceptionFileParam(
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("랜섬웨어 신뢰정보 예외 파일 삭제", "랜섬웨어 신뢰정보 예외 파일 삭제 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
				} else {
					loadFileTreeView();
					displayInfoDialog("랜섬웨어 신뢰정보 예외 파일 삭제", "정상 처리되었습니다.", "정상적으로 랜섬웨어 신뢰정보 예외 파일이 삭제되었습니다.");
				}
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("랜섬웨어 신뢰정보 예외 파일 삭제", "랜섬웨어 신뢰정보 예외 파일 삭제 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
