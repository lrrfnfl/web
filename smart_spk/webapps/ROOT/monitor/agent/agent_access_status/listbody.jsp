...
<!--
	...
	$(document).ready(function() {
		...
		g_objAgentMonitorList = $('#agentmonitor-list');
...
<% } %>
		...
		fillDropdownList(g_objAgentMonitorList.find('#search-condition').find('select[name="searchinstallflag"]'), g_htInstallStateList, null, "전체");
		fillDropdownList(g_objAgentMonitorList.find('#search-condition').find('select[name="searchservicestate"]'), g_htServiceStateList, null, "전체");
...
<% } %>
	...
	loadAgentMonitorList = function() {

		var objSearchCondition = g_objAgentMonitorList.find('#search-condition');
		var objSearchResult = g_objAgentMonitorList.find('#search-result');
		...
		var postData = getRequestAgentMonitorListParam(targetCompanyId,
				arrTargetDeptList,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("사용자 접속 현황 목록 조회", "사용자 접속 현황 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th id="USERNAME" class="ui-state-default">사용자</th>';
				htmlContents += '<th width="80" id="INSTALLFLAG" class="ui-state-default">설치 상태</th>';
				htmlContents += '<th width="80" id="SERVICESTATEFLAG" class="ui-state-default">서비스 상태</th>';
				htmlContents += '<th width="125" id="LASTLOGINDATETIME" class="ui-state-default">최종 로그인 일시</th>';
				htmlContents += '<th width="125" id="ELAPSEDSECONDS" class="ui-state-default">종료 후 경과시간</th>';
				htmlContents += '<th width="90" id="SERVICESTATEFLAG" class="ui-state-default">클라이언트 상태</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
				...
				if (resultRecordCount > 0) {
					$(data).find('record').each( function(index) {
						...
						var loginFlag = $(this).find('loginflag').text();
						var lastLoginDatetime = $(this).find('lastlogindatetime').text();
						var serviceStateFlag = $(this).find('servicestateflag').text();
						var elapsedSeconds = $(this).find('elapsedseconds').text();

						if (lastLoginDatetime.length == 0) {
							lastLoginDatetime = "";
						}
						...
						if (installFlag == '<%=InstallState.INSTALL_STATE_INSTALL%>') {
							if (elapsedSeconds.length != 0) {
								if (parseInt(elapsedSeconds) > <%=CommonConstant.AGENT_CHECK_TIME_LIMIT%> + 1) {
									agentStatus = "종료";
									elapsedTime = getElapsedTimeString(elapsedSeconds);
								} else {
									agentStatus = "실행중";
								}
...
<% } %>
						...
						htmlContents += '<td style="text-align: right;">' + elapsedTime + '</td>';
						htmlContents += '<td style="text-align: center;">' + agentStatus + '</td>';
						htmlContents += '</tr>';
					});
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="8" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자 접속 현황이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="7" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자 접속 현황이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 사용자 접속 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 사용자 접속 현황');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 사용자 접속 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 사용자 접속 현황');
				}

				g_objAgentMonitorList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("사용자 접속 현황 목록 조회", "사용자 접속 현황 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	getElapsedTimeString = function(elapsedSeconds) {
		var elapsedTime = "";

		if (elapsedSeconds.length == 0) {
			elapsedTime = "-";
		} else {
			if (parseInt(elapsedSeconds)/3600 > 1) {
				elapsedTime += parseInt(parseInt(elapsedSeconds)/3600).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + '시간 ';
			}
			if (parseInt(elapsedSeconds)/60 > 1) {
				elapsedTime += parseInt(parseInt(elapsedSeconds)%3600/60) + '분 ';
			}
			elapsedTime += parseInt(elapsedSeconds)%60 + '초';
		}
...
<div class="inner-center">
	<div class="pane-header">전체 사업장 사용자 접속 현황 목록</div>
	<div class="ui-layout-content">
		<div id="agentmonitor-list" style="display: none;">
