...
<% } %>
	...
	loadDetectFileProcessStatusPerUserList = function() {
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("사용자별 검출 파일 처리 현황 조회", "사용자별 검출 파일 처리 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
					...
					htmlContents += '<th rowspan="2" id="USERNAME" class="ui-state-default">사용자</th>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					if (oemName == "MOCOMSYS") {
						htmlContents += '<td colspan="10" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자별 검출 파일 처리 현황이 존재하지 않습니다.</div></td>';
					} else {
						htmlContents += '<td colspan="7" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자별 검출 파일 처리 현황이 존재하지 않습니다.</div></td>';
					}
<% } else { %>
					if (oemName == "MOCOMSYS") {
						htmlContents += '<td colspan="9" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자별 검출 파일 처리 현황이 존재하지 않습니다.</div></td>';
					} else {
						htmlContents += '<td colspan="6" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자별 검출 파일 처리 현황이 존재하지 않습니다.</div></td>';
					}
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 사용자별 검출 파일 처리 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 사용자별 검출 파일 처리 현황');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 사용자별 검출 파일 처리 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 사용자별 검출 파일 처리 현황');
				}
			...
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("사용자별 검출 파일 처리 현황 조회", "사용자별 검출 파일 처리 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};
	...
	downloadDetectFileProcessStatusPerUserList = function() {
		...
		openDownloadDialog("사용자별 검출 파일 처리 현황", "사용자별 검출 파일 처리 현황", postData);
	};
//-->
</script>
...
<div class="inner-center"> 
	<div class="pane-header">전체 사업장 사용자별 검출 파일 처리 현황</div>
