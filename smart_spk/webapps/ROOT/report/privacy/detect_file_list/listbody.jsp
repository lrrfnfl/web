...
<%@ include file ="detectfile-dialog.jsp"%>
<%@ include file ="detectfileextendexpiration-dialog.jsp"%>
<%@ include file ="detectfileexpirationextendhistory-dialog.jsp"%>
<script type="text/javascript">
<!--
	var g_objOrganizationTree;
	var g_objDetectFileList;

	var g_htPatternList = new Hashtable();
	var g_htSearchTypeList = new Hashtable();
	var g_htFileTypeList = new Hashtable();
	var g_htFileCategoryList = new Hashtable();
	var g_htResultStateList = new Hashtable();
	var g_htUserTypeList = new Hashtable();
	var g_htRequesterTypeList = new Hashtable();
	...
	$(document).ready(function() {
		...
		g_objDetectFileList = $('#detectfile-list');
...
<% } %>
		...
		g_htPatternList = loadPatternList();
		...
		g_htFileCategoryList = loadTypeList("FILE_CATEGORY");
		if (g_htFileCategoryList.isEmpty()) {
			displayAlertDialog("파일 분류 유형 조회", "파일 분류 유형 조회 중 오류가 발생하였습니다.", "자세한 사항은 시스템 관리자에게 문의해 주십시오.");
		}
		...
		g_htRequesterTypeList = loadTypeList("REQUESTER_TYPE");
		if (g_htRequesterTypeList.isEmpty()) {
			displayAlertDialog("요청자 유형 조회", "요청자 유형 조회 중 오류가 발생하였습니다.", "자세한 사항은 시스템 관리자에게 문의해 주십시오.");
		}

		fillDropdownList(g_objDetectFileList.find('#search-condition').find('select[name="searchfiletype"]'), g_htFileTypeList, null, "전체");
		fillDropdownList(g_objDetectFileList.find('#search-condition').find('select[name="searchfilecategory"]'), g_htFileCategoryList, null, "전체");
		fillDropdownList(g_objDetectFileList.find('#search-condition').find('select[name="searchresultstate"]'), g_htResultStateList, null, "전체");
		//g_objDetectFileList.find('#search-condition').find('select[name="searchresultstate"]').find('[value="' + <%=ResultState.RESULT_STATE_SEND_SERVER%> + '"]').remove();
		g_objDetectFileList.find('#search-condition').find('select[name="searchresultstate"]').find('[value="' + <%=ResultState.RESULT_STATE_PROCESS_FAIL%> + '"]').remove();
		g_objDetectFileList.find('#search-condition').find('select[name="searchresultstate"]').find('[value="' + <%=ResultState.RESULT_STATE_SEARCH_FAIL%> + '"]').remove();
		...
		$(document).on("click", ".list-table tbody tr", function() { if (typeof $(this).attr('seqno') != typeof undefined) { openDetectFileInfoDialog($(this).attr('seqno')); } });
	});
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				...
				$(data).find('record').each( function() {
					if (!g_htList.containsKey($(this).find('patternid').text())) {
						g_htList.put($(this).find('patternid').text(), $(this).find('patterncategoryname').text());
					}
				});
			},
...
<% } %>
	...
	loadDetectFileList = function() {

		var objSearchCondition = g_objDetectFileList.find('#search-condition');
		var objSearchResult = g_objDetectFileList.find('#search-result');

		var objSearchFileType = objSearchCondition.find('select[name="searchfiletype"]');
		var objSearchFileCategory = objSearchCondition.find('select[name="searchfilecategory"]');
		...
		var postData = getRequestDetectFileListParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchFileType.val(),
				objSearchFileCategory.val(),
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("검출 파일 목록 조회", "검출 파일 목록 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				htmlContents += '<th width="15%" class="ui-state-default">부서</th>';
				htmlContents += '<th width="15%" class="ui-state-default">사용자</th>';
				htmlContents += '<th class="ui-state-default">검출 파일</th>';
				htmlContents += '<th width="60" class="ui-state-default">파일 유형</th>';
				//htmlContents += '<th width="60" class="ui-state-default">파일 분류</th>';
				htmlContents += '<th width="70" class="ui-state-default">최종 검사일</th>';
				...
				if (resultRecordCount > 0) {
					$(data).find('record').each( function(index) {
						...
						var fileCategory = $(this).find('filecategory').text();
						var lastSearchDate = $(this).find('lastsearchdate').text();
						var lastResult = $(this).find('lastresult').text();

						htmlContents += '<tr seqno="' + $(this).find('seqno').text() + '" class="' + lineStyle + '">';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
...
<% } %>
						...
						htmlContents += '<td style="text-align: center;">' + g_htFileTypeList.get(fileType) + '</td>';
						//htmlContents += '<td>' + g_htFileCategoryList.get(fileCategory) + '</td>';
						htmlContents += '<td style="text-align: center;">' + lastSearchDate + '</td>';
						htmlContents += '<td style="text-align: center;">' + g_htResultStateList.get(lastResult) + '</td>';
						htmlContents += '</tr>';
					});
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="7" align="center"><div style="padding: 10px 0; text-align: center;">검출 파일 목록이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="8" align="center"><div style="padding: 10px 0; text-align: center;">검출 파일 목록이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 검출 파일 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 검출 파일 목록');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 검출 파일 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else if (objTreeReference.get_selected().attr("node_type") == 'user') {
					$('.inner-center .pane-header').text('사용자 검출 파일 목록 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 검출 파일 목록');
				}

				g_objDetectFileList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("검출 파일 목록 조회", "검출 파일 목록 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadDetectFileList = function() {
		...
		var postData = getRequestCreateDetectFileListFileParam(
				targetCompanyId,
				arrTargetDeptList,
				targetUserId,
				objSearchFileType.val(),
...
<div class="inner-center">
	<div class="pane-header">전체 사업장 검출 현황 목록</div>
	<div class="ui-layout-content">
		<div id="detectfile-list" style="display: none;">
			<div id="search-condition" class="inline-search-condition">
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>파일 유형
				<select id="searchfiletype" name="searchfiletype" class="ui-widget-content"></select>
				<ul class="ui-helper-clearfix icons" style="display: none;"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>파일 분류
				<select id="searchfilecategory" name="searchfilecategory" class="ui-widget-content" style="display: none;"></select>
				...
				<ul class="ui-helper-clearfix icons"><li><span class="ui-icon ui-icon-triangle-1-e"></span></li></ul>검사 기간
