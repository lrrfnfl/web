...
<!--
	...
	var g_objSafeExportStatusPerDeptList;
	...
	$(document).ready(function() {
		...
		g_objSafeExportStatusPerDeptList = $('#safeexportstatusperdept-list');
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
		...
		loadCompanyTreeView();
<% } else { %>
		loadSafeExportStatusPerDeptList();
<% } %>
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				downloadSafeExportStatusPerDeptList();
			}
		});
...
<% } %>
	...
	loadSafeExportStatusPerDeptList = function() {

		var objSearchCondition = g_objSafeExportStatusPerDeptList.find('#search-condition');
		var objSearchResult = g_objSafeExportStatusPerDeptList.find('#search-result');
...
<% } %>
		...
		var postData = getRequestSafeExportStatusPerDeptListParam(targetCompanyId,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("부서별 반출 현황 조회", "부서별 반출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="7" align="center"><div style="padding: 10px 0; text-align: center;">등록된 부서별 반출 현황이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="6" align="center"><div style="padding: 10px 0; text-align: center;">등록된 부서별 반출 현황이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
				if (objTreeReference.get_selected().attr("node_type") == 'company') {
					$('.inner-center .pane-header').text('사업장 부서별 반출 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 부서별 반출 현황');
				}
<% } else { %>
				$('.inner-center .pane-header').text('부서별 반출 현황');
<% } %>
				g_objSafeExportStatusPerDeptList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("부서별 반출 현황 조회", "부서별 반출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadSafeExportStatusPerDeptList = function() {
...
<% } %>
		...
		var postData = getRequestCreateSafeExportStatusPerDeptListFileParam(targetCompanyId,
				objSearchDateFrom.val(),
				objSearchDateTo.val(),
				g_searchListOrderByName,
				g_searchListOrderByDirection);

		openDownloadDialog("부서별 반출 현황", postData);
	};
//-->
</script>
...
<div class="inner-center">
	...
	<div class="ui-layout-content">
		<div id="safeexportstatusperdept-list" style="display: none;">
