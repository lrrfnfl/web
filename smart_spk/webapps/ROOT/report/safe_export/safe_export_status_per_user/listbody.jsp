...
<!--
	...
	var g_objSafeExportStatusPerUserList;
	...
	$(document).ready(function() {
		...
		g_objSafeExportStatusPerUserList = $('#safeexportstatusperuser-list');
...
<% } %>
		...
		$('button').click( function () {
			if ($(this).attr('id') == 'btnSearch') {
				...
				loadSafeExportStatusPerUserList();
			} else if ($(this).attr('id') == 'btnDownload') {
				downloadSafeExportStatusPerUserList();
			}
		});
...
<% } %>
	...
	loadSafeExportStatusPerUserList = function() {

		var objSearchCondition = g_objSafeExportStatusPerUserList.find('#search-condition');
		var objSearchResult = g_objSafeExportStatusPerUserList.find('#search-result');
		...
		var postData = getRequestSafeExportStatusPerUserListParam(targetCompanyId,
				arrTargetDeptList,
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if ($(data).find('errorcode').text() != "0000") {
					displayAlertDialog("사용자별 반출 현황 조회", "사용자별 반출 현황 조회 중 오류가 발생하였습니다.", $(data).find('errormsg').text());
					return false;
				}
...
<% } %>
				...
				htmlContents += '<th id="EXPORTCOUNT" class="ui-state-default">반출 요청 수</th>';
				htmlContents += '<th id="EXPORTFILESCOUNT" class="ui-state-default">반출 파일 수</th>';
				htmlContents += '<th id="DECODEDCOUNT" class="ui-state-default">복호화 요청 수</th>';
				htmlContents += '<th id="DECODEDFILESCOUNT" class="ui-state-default">복호화 파일 수</th>';
				htmlContents += '</tr>';
				htmlContents += '</thead>';
				htmlContents += '<tbody>';
				...
				if (resultRecordCount > 0) {
					$(data).find('record').each( function(index) {
						...
						var exportCount = $(this).find('exportcount').text();
						var exportFilesCount = $(this).find('exportfilescount').text();
						var decodedCount = $(this).find('decodedcount').text();
						var decodedFilesCount = $(this).find('decodedfilescount').text();
...
<% } %>
						...
						htmlContents += '<td style="text-align: right;">' + exportCount.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' 건</td>';
						htmlContents += '<td style="text-align: right;">' + exportFilesCount.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' 파일</td>';
						htmlContents += '<td style="text-align: right;">' + decodedCount.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' 건</td>';
						htmlContents += '<td style="text-align: right;">' + decodedFilesCount.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' 파일</td>';
						htmlContents += '</tr>';
					});
					...
					htmlContents += '<tr>';
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					htmlContents += '<td colspan="7" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자별 반출 현황이 존재하지 않습니다.</div></td>';
<% } else { %>
					htmlContents += '<td colspan="6" align="center"><div style="padding: 10px 0; text-align: center;">등록된 사용자별 반출 현황이 존재하지 않습니다.</div></td>';
<% } %>
					htmlContents += '</tr>';
...
<% if (AdminType.ADMIN_TYPE_SITE_ADMIN.equals((String)session.getAttribute("ADMINTYPE"))) { %>
					$('.inner-center .pane-header').text('사업장 사용자별 반출 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
<% } else { %>
					$('.inner-center .pane-header').text('전체 사용자별 반출 현황');
<% } %>
				} else if (objTreeReference.get_selected().attr("node_type") == 'dept') {
					$('.inner-center .pane-header').text('부서 사용자별 반출 현황 - [' + objTreeReference.get_text(objTreeReference.get_selected()) + ']');
				} else {
					$('.inner-center .pane-header').text('전체 사업장 사용자별 반출 현황');
				}

				g_objSafeExportStatusPerUserList.show();
			},
			error: function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status != 0 && jqXHR.readyState != 0) {
					displayAlertDialog("사용자별 반출 현황 조회", "사용자별 반출 현황 조회 중 오류가 발생하였습니다.", jqXHR.statusText + "(" + jqXHR.status + ")");
				}
			}
		});
	};

	downloadSafeExportStatusPerUserList = function() {
		...
		var postData = getRequestCreateSafeExportStatusPerUserListFileParam(targetCompanyId,
				arrTargetDeptList,
				objSearchDateFrom.val(),
				objSearchDateTo.val(),
				g_searchListOrderByName,
		...
		openDownloadDialog("사용자별 반출 현황", postData);
	};
//-->
</script>
...
<div class="inner-center"> 
	<div class="pane-header">전체 사업장 반출 현황 목록</div>
	<div class="ui-layout-content">
		<div id="safeexportstatusperuser-list" style="display: none;">
