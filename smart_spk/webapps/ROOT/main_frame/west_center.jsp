...
<!--
	var g_westCenterChart = null;

	$(document).ready( function() {
		$('#tab-westcenter').tabs({
			heightStyle: "auto",
			activate: function(event, ui) {
				loadWestCenterChartData(ui.newTab.index());
			}
		});

		loadWestCenterChartData(TAB_DAILY);
	});

	loadWestCenterChartData = function(selectedTab) {
...
<% } %>
		...
		var postData = getRequestSearchStatusPerTermsParam(companyId,
				'',
				'',
				searchTermType,
	...
	loadWestCenterChart = function(selectedTab, responseData) {
...
// 				],
		...
		var titleText;
		if (selectedTab == TAB_DAILY) {
			ctx = document.getElementById('chart-westcenter-tab1').getContext('2d');
			titleText = "검사 현황(최근 1주일)";
		} else if (selectedTab == TAB_MONTHLY) {
			ctx = document.getElementById('chart-westcenter-tab2').getContext('2d');
			titleText = "검사 현황(최근 6개월)";
		}

		if (g_westCenterChart != null) g_westCenterChart.destroy();

		g_westCenterChart = new Chart(ctx, {
			type: 'bar',
			data: chartData,
			options: {
				elements: {
...
<div class="ui-layout-center zero-padding">
	<div class="pane-header">기간별 검사 현황</div>
	<div class="ui-layout-content zero-padding">
		<div id="tab-westcenter" class="styles-tab border-none">
			<ul>
				<li><a href="#tab-westcenter-1">일별 검사 현황</a></li>
				<li><a href="#tab-westcenter-2">월별 검사 현황</a></li>
			</ul>
			<div id="tab-westcenter-1">
				<div class="chart-container" style="width: 100%; height: 240px;"><canvas id="chart-westcenter-tab1"></canvas></div>
			</div>
			<div id="tab-westcenter-2">
				<div class="chart-container" style="width: 100%; height: 240px;"><canvas id="chart-westcenter-tab2"></canvas></div>
			</div>
		</div>
	</div>
</div>
